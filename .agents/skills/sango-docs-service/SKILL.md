...
此技能提供一組流程，協助你把目前 repo 內 `docs/3yWebsite/docs` 的 Markdown 與 `docs/data/*.json` 轉化為 `merc-area-builder`、`merc-source-explainer` 等任務可直接引用的回答或資料。遇到需要說明 Sango3838 世界觀、公告、技能、國家系統或地圖交通時，優先依照下列步驟行事。
...
## 2. 常見任務映射（面向 Merc-FJU 3.0 skills）
- **規劃新區域故事/Serial**：
...
常見對接技能：
- `merc-area-builder`：世界觀、技能、國家、交通、下載資料
- `merc-source-explainer`：解釋 `system.md`、`realm.md`、`skills.md` 等 docs 如何對應目前程式與資料目錄

## 3. 引用與來源標註
- 以「> 原文：相對路徑」標示來源（遵循 docs 內的慣例），確保回答可追溯。
- 當回答與 `merc-area-builder` 流程直接相關，補充它會影響 `area/directory.lst`、room / NPC / transport 提示、或相關 `data/*.txt` / help 文字的哪一步，並說明該資訊在 docs 哪一節。
...
- 若是對 `merc-source-explainer` 提供資料，優先補「docs 對應哪個 `src/*.c` / `area/` / `data/` 模組」的說明，而不只摘錄世界觀文字。
